...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  } else if (message.type === 'JOURNAL_ENTRY_ADDED') {
    console.log('Received new journal entry from background:', message.entry);
    
    // Add the entry to the local journalMessages array
    if (message.entry) {
      journalMessages.push(message.entry);
      
      // If we're on the journal tab, update the display immediately
      if (currentTab === 'journal') {
        renderJournalMessages();
      }
      
      console.log('Updated journal with new entry via keyboard shortcut');
    }
